...
async function main() {
	try {
		...
		try {
            ...
            result = await statefulTxn.submit(assetID,10,'apples');
			console.log(" Asset Was created. Public details should be present !");
            await sleep(3000);

            //await contractOrg1.submitTransaction('InitLedger');
            console.log('Expires test');
            result = await contractOrg1.evaluateTransaction('ExpiresIn',assetID);
			//console.log(`*** Result: ${Buffer.from(JSON.parse(result).data).toString('utf8')}`);
			...
			result = await contractOrg1.evaluateTransaction('ReadAssetWithExpiration', assetID);//should delete let
			...
			try {
                ...
                const asset_price = {
					asset_id: assetID,
					price: 110,
					trade_id: randomNumber,
                    // timestamp:transaction.getTxTimestamp(),
